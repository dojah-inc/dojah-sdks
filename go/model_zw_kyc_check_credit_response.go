/*
DOJAH Publilc APIs

Use Dojah to verify, onboard and manage user identity across Africa!

API version: 1.0.0
*/

// Code generated by Konfig (https://konfigthis.com); DO NOT EDIT.

package dojah

import (
	"encoding/json"
)

// ZwKycCheckCreditResponse struct for ZwKycCheckCreditResponse
type ZwKycCheckCreditResponse struct {
	Entity *ZwKycCheckCreditResponseEntity `json:"entity,omitempty"`
}

// NewZwKycCheckCreditResponse instantiates a new ZwKycCheckCreditResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewZwKycCheckCreditResponse() *ZwKycCheckCreditResponse {
	this := ZwKycCheckCreditResponse{}
	return &this
}

// NewZwKycCheckCreditResponseWithDefaults instantiates a new ZwKycCheckCreditResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewZwKycCheckCreditResponseWithDefaults() *ZwKycCheckCreditResponse {
	this := ZwKycCheckCreditResponse{}
	return &this
}

// GetEntity returns the Entity field value if set, zero value otherwise.
func (o *ZwKycCheckCreditResponse) GetEntity() ZwKycCheckCreditResponseEntity {
	if o == nil || isNil(o.Entity) {
		var ret ZwKycCheckCreditResponseEntity
		return ret
	}
	return *o.Entity
}

// GetEntityOk returns a tuple with the Entity field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ZwKycCheckCreditResponse) GetEntityOk() (*ZwKycCheckCreditResponseEntity, bool) {
	if o == nil || isNil(o.Entity) {
    return nil, false
	}
	return o.Entity, true
}

// HasEntity returns a boolean if a field has been set.
func (o *ZwKycCheckCreditResponse) HasEntity() bool {
	if o != nil && !isNil(o.Entity) {
		return true
	}

	return false
}

// SetEntity gets a reference to the given ZwKycCheckCreditResponseEntity and assigns it to the Entity field.
func (o *ZwKycCheckCreditResponse) SetEntity(v ZwKycCheckCreditResponseEntity) {
	o.Entity = &v
}

func (o ZwKycCheckCreditResponse) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Entity) {
		toSerialize["entity"] = o.Entity
	}
	return json.Marshal(toSerialize)
}

type NullableZwKycCheckCreditResponse struct {
	value *ZwKycCheckCreditResponse
	isSet bool
}

func (v NullableZwKycCheckCreditResponse) Get() *ZwKycCheckCreditResponse {
	return v.value
}

func (v *NullableZwKycCheckCreditResponse) Set(val *ZwKycCheckCreditResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableZwKycCheckCreditResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableZwKycCheckCreditResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableZwKycCheckCreditResponse(val *ZwKycCheckCreditResponse) *NullableZwKycCheckCreditResponse {
	return &NullableZwKycCheckCreditResponse{value: val, isSet: true}
}

func (v NullableZwKycCheckCreditResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableZwKycCheckCreditResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


