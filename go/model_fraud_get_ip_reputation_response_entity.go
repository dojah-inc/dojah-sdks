/*
DOJAH Publilc APIs

Use Dojah to verify, onboard and manage user identity across Africa!

API version: 1.0.0
*/

// Code generated by Konfig (https://konfigthis.com); DO NOT EDIT.

package dojah

import (
	"encoding/json"
)

// FraudGetIpReputationResponseEntity struct for FraudGetIpReputationResponseEntity
type FraudGetIpReputationResponseEntity struct {
	Report *FraudGetIpReputationResponseEntityReport `json:"report,omitempty"`
	Success *bool `json:"success,omitempty"`
}

// NewFraudGetIpReputationResponseEntity instantiates a new FraudGetIpReputationResponseEntity object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFraudGetIpReputationResponseEntity() *FraudGetIpReputationResponseEntity {
	this := FraudGetIpReputationResponseEntity{}
	return &this
}

// NewFraudGetIpReputationResponseEntityWithDefaults instantiates a new FraudGetIpReputationResponseEntity object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFraudGetIpReputationResponseEntityWithDefaults() *FraudGetIpReputationResponseEntity {
	this := FraudGetIpReputationResponseEntity{}
	return &this
}

// GetReport returns the Report field value if set, zero value otherwise.
func (o *FraudGetIpReputationResponseEntity) GetReport() FraudGetIpReputationResponseEntityReport {
	if o == nil || isNil(o.Report) {
		var ret FraudGetIpReputationResponseEntityReport
		return ret
	}
	return *o.Report
}

// GetReportOk returns a tuple with the Report field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FraudGetIpReputationResponseEntity) GetReportOk() (*FraudGetIpReputationResponseEntityReport, bool) {
	if o == nil || isNil(o.Report) {
    return nil, false
	}
	return o.Report, true
}

// HasReport returns a boolean if a field has been set.
func (o *FraudGetIpReputationResponseEntity) HasReport() bool {
	if o != nil && !isNil(o.Report) {
		return true
	}

	return false
}

// SetReport gets a reference to the given FraudGetIpReputationResponseEntityReport and assigns it to the Report field.
func (o *FraudGetIpReputationResponseEntity) SetReport(v FraudGetIpReputationResponseEntityReport) {
	o.Report = &v
}

// GetSuccess returns the Success field value if set, zero value otherwise.
func (o *FraudGetIpReputationResponseEntity) GetSuccess() bool {
	if o == nil || isNil(o.Success) {
		var ret bool
		return ret
	}
	return *o.Success
}

// GetSuccessOk returns a tuple with the Success field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FraudGetIpReputationResponseEntity) GetSuccessOk() (*bool, bool) {
	if o == nil || isNil(o.Success) {
    return nil, false
	}
	return o.Success, true
}

// HasSuccess returns a boolean if a field has been set.
func (o *FraudGetIpReputationResponseEntity) HasSuccess() bool {
	if o != nil && !isNil(o.Success) {
		return true
	}

	return false
}

// SetSuccess gets a reference to the given bool and assigns it to the Success field.
func (o *FraudGetIpReputationResponseEntity) SetSuccess(v bool) {
	o.Success = &v
}

func (o FraudGetIpReputationResponseEntity) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Report) {
		toSerialize["report"] = o.Report
	}
	if !isNil(o.Success) {
		toSerialize["success"] = o.Success
	}
	return json.Marshal(toSerialize)
}

type NullableFraudGetIpReputationResponseEntity struct {
	value *FraudGetIpReputationResponseEntity
	isSet bool
}

func (v NullableFraudGetIpReputationResponseEntity) Get() *FraudGetIpReputationResponseEntity {
	return v.value
}

func (v *NullableFraudGetIpReputationResponseEntity) Set(val *FraudGetIpReputationResponseEntity) {
	v.value = val
	v.isSet = true
}

func (v NullableFraudGetIpReputationResponseEntity) IsSet() bool {
	return v.isSet
}

func (v *NullableFraudGetIpReputationResponseEntity) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFraudGetIpReputationResponseEntity(val *FraudGetIpReputationResponseEntity) *NullableFraudGetIpReputationResponseEntity {
	return &NullableFraudGetIpReputationResponseEntity{value: val, isSet: true}
}

func (v NullableFraudGetIpReputationResponseEntity) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFraudGetIpReputationResponseEntity) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


