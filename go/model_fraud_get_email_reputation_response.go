/*
DOJAH Publilc APIs

Use Dojah to verify, onboard and manage user identity across Africa!

API version: 1.0.0
*/

// Code generated by Konfig (https://konfigthis.com); DO NOT EDIT.

package dojah

import (
	"encoding/json"
)

// FraudGetEmailReputationResponse struct for FraudGetEmailReputationResponse
type FraudGetEmailReputationResponse struct {
	Entity *FraudGetEmailReputationResponseEntity `json:"entity,omitempty"`
}

// NewFraudGetEmailReputationResponse instantiates a new FraudGetEmailReputationResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFraudGetEmailReputationResponse() *FraudGetEmailReputationResponse {
	this := FraudGetEmailReputationResponse{}
	return &this
}

// NewFraudGetEmailReputationResponseWithDefaults instantiates a new FraudGetEmailReputationResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFraudGetEmailReputationResponseWithDefaults() *FraudGetEmailReputationResponse {
	this := FraudGetEmailReputationResponse{}
	return &this
}

// GetEntity returns the Entity field value if set, zero value otherwise.
func (o *FraudGetEmailReputationResponse) GetEntity() FraudGetEmailReputationResponseEntity {
	if o == nil || isNil(o.Entity) {
		var ret FraudGetEmailReputationResponseEntity
		return ret
	}
	return *o.Entity
}

// GetEntityOk returns a tuple with the Entity field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FraudGetEmailReputationResponse) GetEntityOk() (*FraudGetEmailReputationResponseEntity, bool) {
	if o == nil || isNil(o.Entity) {
    return nil, false
	}
	return o.Entity, true
}

// HasEntity returns a boolean if a field has been set.
func (o *FraudGetEmailReputationResponse) HasEntity() bool {
	if o != nil && !isNil(o.Entity) {
		return true
	}

	return false
}

// SetEntity gets a reference to the given FraudGetEmailReputationResponseEntity and assigns it to the Entity field.
func (o *FraudGetEmailReputationResponse) SetEntity(v FraudGetEmailReputationResponseEntity) {
	o.Entity = &v
}

func (o FraudGetEmailReputationResponse) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Entity) {
		toSerialize["entity"] = o.Entity
	}
	return json.Marshal(toSerialize)
}

type NullableFraudGetEmailReputationResponse struct {
	value *FraudGetEmailReputationResponse
	isSet bool
}

func (v NullableFraudGetEmailReputationResponse) Get() *FraudGetEmailReputationResponse {
	return v.value
}

func (v *NullableFraudGetEmailReputationResponse) Set(val *FraudGetEmailReputationResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableFraudGetEmailReputationResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableFraudGetEmailReputationResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFraudGetEmailReputationResponse(val *FraudGetEmailReputationResponse) *NullableFraudGetEmailReputationResponse {
	return &NullableFraudGetEmailReputationResponse{value: val, isSet: true}
}

func (v NullableFraudGetEmailReputationResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFraudGetEmailReputationResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


