/*
DOJAH Publilc APIs

Use Dojah to verify, onboard and manage user identity across Africa!

API version: 1.0.0
*/

// Code generated by Konfig (https://konfigthis.com); DO NOT EDIT.

package dojah

import (
	"encoding/json"
)

// AmlScreenAmlResponseEntity struct for AmlScreenAmlResponseEntity
type AmlScreenAmlResponseEntity struct {
	ReferenceId *string `json:"reference_id,omitempty"`
	Status *string `json:"status,omitempty"`
}

// NewAmlScreenAmlResponseEntity instantiates a new AmlScreenAmlResponseEntity object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAmlScreenAmlResponseEntity() *AmlScreenAmlResponseEntity {
	this := AmlScreenAmlResponseEntity{}
	return &this
}

// NewAmlScreenAmlResponseEntityWithDefaults instantiates a new AmlScreenAmlResponseEntity object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAmlScreenAmlResponseEntityWithDefaults() *AmlScreenAmlResponseEntity {
	this := AmlScreenAmlResponseEntity{}
	return &this
}

// GetReferenceId returns the ReferenceId field value if set, zero value otherwise.
func (o *AmlScreenAmlResponseEntity) GetReferenceId() string {
	if o == nil || isNil(o.ReferenceId) {
		var ret string
		return ret
	}
	return *o.ReferenceId
}

// GetReferenceIdOk returns a tuple with the ReferenceId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmlScreenAmlResponseEntity) GetReferenceIdOk() (*string, bool) {
	if o == nil || isNil(o.ReferenceId) {
    return nil, false
	}
	return o.ReferenceId, true
}

// HasReferenceId returns a boolean if a field has been set.
func (o *AmlScreenAmlResponseEntity) HasReferenceId() bool {
	if o != nil && !isNil(o.ReferenceId) {
		return true
	}

	return false
}

// SetReferenceId gets a reference to the given string and assigns it to the ReferenceId field.
func (o *AmlScreenAmlResponseEntity) SetReferenceId(v string) {
	o.ReferenceId = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *AmlScreenAmlResponseEntity) GetStatus() string {
	if o == nil || isNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmlScreenAmlResponseEntity) GetStatusOk() (*string, bool) {
	if o == nil || isNil(o.Status) {
    return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *AmlScreenAmlResponseEntity) HasStatus() bool {
	if o != nil && !isNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *AmlScreenAmlResponseEntity) SetStatus(v string) {
	o.Status = &v
}

func (o AmlScreenAmlResponseEntity) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.ReferenceId) {
		toSerialize["reference_id"] = o.ReferenceId
	}
	if !isNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	return json.Marshal(toSerialize)
}

type NullableAmlScreenAmlResponseEntity struct {
	value *AmlScreenAmlResponseEntity
	isSet bool
}

func (v NullableAmlScreenAmlResponseEntity) Get() *AmlScreenAmlResponseEntity {
	return v.value
}

func (v *NullableAmlScreenAmlResponseEntity) Set(val *AmlScreenAmlResponseEntity) {
	v.value = val
	v.isSet = true
}

func (v NullableAmlScreenAmlResponseEntity) IsSet() bool {
	return v.isSet
}

func (v *NullableAmlScreenAmlResponseEntity) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAmlScreenAmlResponseEntity(val *AmlScreenAmlResponseEntity) *NullableAmlScreenAmlResponseEntity {
	return &NullableAmlScreenAmlResponseEntity{value: val, isSet: true}
}

func (v NullableAmlScreenAmlResponseEntity) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAmlScreenAmlResponseEntity) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


